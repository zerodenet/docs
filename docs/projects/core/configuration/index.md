...
`1500`，最小 `576`。
启动 TUN 时若 `tun.start`/CLI 显式传入 `mtu`，该值覆盖全局配置；否则使用这里的值。
网络栈会按 MTU 扣除 IPv6 与 TCP 头部后计算通告 MSS，避免生成超过配置 MTU 的 TCP 包。
当前 Windows/Linux/macOS TUN 后端的系统网卡配置实现仍是空操作，因此如果还需要同步修改
操作系统网卡本身的 MTU，宿主侧仍需配置对应接口；本参数已经完整控制 Zero 用户态网络层。
...
`api.event_sinks` 描述归一化事件的投递目标。事件类型必须来自 [api.md](https://github.com/zerodenet/core/blob/develop/docs/project/api.md) 中的事件目录。
...
UDP 出站选择由内核 UDP 分发路径处理。当前 TCP、UDP、MUX、传输和限制事实通过 `capabilities.protocols` 暴露，并记录在 [protocol-capabilities.md](/projects/core/reference/protocol-capabilities) 中。
...
- 当前仅支持 `http://` 探测 URL
- `url` 可省略；省略时使用 `runtime.latency_test_url`
- 选择探测成功且延迟最低的成员
...
- `format = domain_list`
- `format = cidr_list`
- `format = zero_rule_ir`（Zero Rule IR v1 JSON）
- `format = zrs`（ZRS 0.1 二进制 MatcherSet）
...
- `zero_rule_ir` 按 [Zero Rule IR v1](/projects/core/reference/zero-rule-ir-v1) 解码并编译为内存 matcher；兼容别名为 `zero_ir`
- `zrs` 按 [ZRS 0.1](/projects/core/reference/zrs-0.1) 执行完整 checksum 校验后只读 mmap，并预热索引根页
- 空行被忽略
- 以 `#` 或 `//` 开头的行被忽略
- 规则文件仅包含匹配数据，不包含动作
- 四种格式最终都通过同一个 `zero-rule` matcher 接口参与 TCP/UDP 的实际 route 首条命中决策；`RouteRule` 的顺序、action 和 final 仍由主配置拥有
- `domain_list` 中每一项保持原有语义：同时匹配该域名本身及其子域名；`cidr_list` 同时支持 IPv4 与 IPv6
