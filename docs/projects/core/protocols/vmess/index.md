...
| 文档 | 对应源码 | 内容 |
|------|---------|------|
| [inbound.md](/projects/core/protocols/vmess/inbound) | `inbound.rs` | `VmessInbound`、`VmessAccept`、`VmessUser`、多用户认证 |
| [outbound.md](/projects/core/protocols/vmess/outbound) | `outbound.rs` | `VmessOutbound`、`VmessOutboundSession`、tunnel targets |
| [shared.md](/projects/core/protocols/vmess/shared) | `shared.rs` | `VmessCipher`、地址编码、UUID 解析 |
| [crypto.md](/projects/core/protocols/vmess/crypto) | `crypto.rs` | Xray AEAD header seal/open、`BodyAead`、KDF |
| [stream.md](/projects/core/protocols/vmess/stream) | `stream.rs` | `VmessAeadStream` 双向 AEAD body relay |
| [mux.md](/projects/core/protocols/vmess/mux) | `mux.rs` | `MuxFrame`、`VmessMuxStream`、Mux.Cool 连接池 |
| [udp.md](/projects/core/protocols/vmess/udp) | `udp.rs` | `VmessUdpPacket`、payload 模式、response bridge |
| [metadata.md](/projects/core/protocols/vmess/metadata) | `metadata.rs` | `VmessProtocol` 能力描述符、limitations |
