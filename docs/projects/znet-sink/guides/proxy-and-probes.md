...
2. 每行填写一个自定义 IP、CIDR 或域名，例如 `192.168.50.0/24`、`intranet.example.com`、`*.example.com`；留空表示没有自定义规则。
...
TUN 来源、运行中修改、网络切换和停止确认见 [TUN 接管与网络切换](./tun)。当前客户端已提供 DNS 配置及 DNS 劫持；先保存有效 DNS 设置，再按 [DNS 与 Fake-IP](./dns)启用。
