...
本页说明 ZNet Sink 如何准备本地代理入口、接管系统代理、打开代理终端，以及如何展示节点和 URLTest 的测速结果。TUN 的独立运行时和路由接管见[TUN 模式](./tun)。
...
ZNet Sink `0.0.16-rc.8` 允许系统代理与 TUN 同时保持开启。两者是独立入口：系统代理继续指向本地 Mixed，其他被系统路由接管的流量可以从 TUN 进入 Zero。排障时应先确定目标应用实际走了哪一个入口。
...
`0.0.16-rc.8` 进一步统一了 URLTest 的节点状态投影：手动探测、策略组快照和配置切换后的显示都绑定当前配置身份，旧配置的测速结果不应继续覆盖新配置节点。
...
本地代理无法使用时，先在“设置 → 常规”确认代理端口，再检查[故障排查](./troubleshooting)。TUN 无流量时不要套用系统代理排查路径，改按[TUN 模式](./tun)检查权限、所有权和底层出口。
