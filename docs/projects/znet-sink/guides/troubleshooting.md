...
`0.0.16-rc.8` 的 TUN 运行需要兼容的 Zero 0.0.16，并需要操作系统允许创建虚拟网卡和修改路由。先检查 TUN 状态中显示的配置来源，再判断应该修改当前配置还是客户端缺省值。

常见情况：

- **权限不足**：Windows 使用管理员权限运行；Linux/macOS 确保当前启动方式具备 TUN 与路由操作权限。客户端会尽量把权限错误直接显示出来；
- **Windows 提示 Wintun 问题**：官方 Zero 0.0.16 Windows 发布包已经包含 Wintun。只有自行打包或文件不完整时才应优先检查组件缺失；
...
本地管理的 TUN 正在运行时，启动参数会被锁定；先关闭 TUN 再修改。

## TUN 已开启但仍有 DNS/WebRTC 泄露

不要把“TUN 已开启”等同于“DNS 劫持已开启”。`0.0.16-rc.8` 暂时禁用了 ZNet Sink 自己管理的 DNS hijack 开关，因为客户端尚未提供完整的 Zero DNS 配置；设置页会显示“DNS 劫持（暂不可用）”。

因此：

- 本地缺省 TUN 当前固定以 `dnsHijack: false` 启动；
- 活动配置如果自行定义 `runtime.tun`，其 TUN/DNS 行为由配置和 Zero 决定，不受本地禁用开关覆盖；
- 仅开启系统代理不会接管所有 UDP、WebRTC 或 DNS；
- 需要确认 DNS/Fake IP 时，应检查实际活动配置和内核运行能力，而不是只看客户端开关。
...
`0.0.16-rc.8` 手动刷新 URLTest 时按当前策略的实际生效出站执行探测。如果 UI 仍显示旧选中项，优先检查策略事件/快照链路，而不是仅重复测速。

不要用连续点击或高频轮询代替事件链路排查。完整语义见[本地代理、系统代理与节点测速](./proxy-and-probes)。

## 实时连接切换配置后异常

切换配置会清空旧配置的页面投影，但新的 flow 事件仍应持续进入。`0.0.16-rc.8` 会在事件流持续运行时同步活动连接快照，并按实际 flow 变化统计暂停期间的更新。
