...
- 严格的 `ZNet-Sink/<版本>`，例如 `ZNet-Sink/0.0.16-rc.8`；
- Clash / Mihomo；
- sing-box。
...
`v0.0.1-dev.26` 增加了面向常见第三方客户端的内置节点订阅模板：

| 模板 | 当前输出边界 |
|------|--------------|
| Shadowrocket | 标准分享链接订阅；支持常用 VMess、VLESS、Trojan、Shadowsocks、Hysteria2 节点 |
| Quantumult X | `server_remote` 资源；当前生成 Shadowsocks、VMess、Trojan 节点 |
| v2rayN | 标准分享链接订阅；支持常用 VMess、VLESS、Trojan、Shadowsocks、Hysteria2 节点 |

这些模板属于**节点订阅**：只导出兼容节点和凭据，不把 Zboard 的 `DIRECT`、`REJECT`、策略组或规则集硬塞进客户端节点列表。相关分流策略由客户端自己的配置负责。

Zero、Clash/Mihomo、sing-box 等完整配置模板仍可以管理策略组和规则。对于 selector 中的特殊目标，`DIRECT` 和 `REJECT` 现在是显式模板选项；是否包含它们由模板配置决定，而不是依赖字符串碰巧出现在成员数组中。

新增内置模板只会补齐缺失的默认记录，不应覆盖管理员已经修改过的模板配置。
...
Zboard 会为不同完整配置客户端生成可直接运行的本地入口：
...
手动 selector 可以按模板配置显式包含 `DIRECT` 和 `REJECT`。URLTest 和 fallback 组不会把它们作为可探测节点，因为它们不是实际代理目标。
...
当前趋势接口支持更细的分钟级时间桶，用于短时间范围内观察连接与流量变化；较长范围仍应使用适合该范围的聚合粒度。前端图表会根据时间桶直接绘制，不通过重复请求全量流量明细模拟“实时趋势”。

principal flow 观测把活动连接、完成连接和流量事实按订阅用户等业务 scope 聚合。没有 principal flow 覆盖的历史时间段应表达为“未覆盖/暂无数据”，而不是把缺失事实强制显示为 0。

后台存储和计费使用精确字节。界面根据数值自动显示 B、KB、MB 或 GB；单位变化只影响显示，不改变数据库、API 或计费结果。

趋势中的日历边界和时间标签遵循系统时区。系统时区的配置语义见[运营概览与系统设置](./operations)。
...
6. 完整配置模板生成的内容包含预期本地入口；
7. 节点组至少包含已启用、可发布的协议服务；
8. 引用的托管规则 ZRS URL 可读取且与当前数据库快照匹配。

第三方节点订阅模板导入失败时，还应确认目标协议是否在该模板的支持范围内。某个协议不受该客户端模板支持时，Zboard 不应通过生成一个格式错误的分享链接来伪装兼容。
