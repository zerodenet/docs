...
删除仅移除面板管理记录，不向 Cloudflare 发起删除请求；供应商上的真实解析会保留。供应商鉴权失败或不可达不会阻止本地记录删除。同一记录有同步操作正在运行时，仍不允许并发删除。

如果要撤销公网解析，应在供应商侧明确删除对应记录，再核对实际解析结果。更改资产身份后重新创建时，也要检查旧远端记录是否仍存在，避免把本地重建当作远端迁移完成。
...
当前 main 删除 DNS 记录只移除面板管理记录，保留供应商真实解析；删除托管证书移除面板记录、关联和面板自动续期，但保留节点证书、私钥与 Certbot 续期配置，也不会向 CA 撤销证书。供应商或 SSH 不可达不会阻止本地删除。

退役时应另外核对供应商解析、节点文件及续期任务。不要把面板列表中消失解释成远端资源已清理。正在执行的同步或签发任务仍需先结束，详见[资源清理边界](https://github.com/zerodenet/zboard/blob/e1b7246cc4ef805bf39b22d634ba209114eb3b14/docs/node-cleanup.md)。
